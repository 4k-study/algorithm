...
def solution():
    ...
    def validate(now, num):
        x, y = blank[now]
        for c in range(9):
            if c == y: continue
            if graph[x][c] == 0: continue
            if graph[x][c] == num: return False

        for r in range(9):
            if r == x: continue
            if graph[r][y] == 0: continue
            if graph[r][y] == num: return False
